...
| Assumed data (pots, bistros, timezones), scenarios | Code (`trestle/assumptions.ts`, `scenarios/seed/`) | Static and reviewable in a diff |
...
| **Manage deals** | Tania edits deals in her **draft** (change → check who's affected → publish). No ticket |
...
2. Schnitzel Tuesday = schnitzel + **a pot**. Pot products (`PRD-0109/0110`, $8.00/$8.50) are **assumed**; the catalogue has none.
...
| Change | Where |
|---|---|
| "Ray keeps his 10% on food specials" | Rulebook → Member Discount → tick *Schnitzel Tuesday* under "Can be added on top of" (config, no code) |
